<template>
  <div class="card p-1">
    ...
    <p class="card-item">
      <button @click="() => setUpdateMethod(methodItem)">update {{ updateSwitch ? "close" : "view" }}</button>
    </p>
    <p class="card-item"><button @click="() => removeMethodAction(methodItem.id)">remove</button></p>
...
export default defineNuxtComponent({
  ...
  setup(props, { emit }) {
    ...
    const updateMethodAction = async () => {
      const updateItem = {...methodItem.value, title: cardTitle.value, body: cardBody.value};
      try {
